...
Move Hubb know that everyone has different reasons for moving. That’s why they have ranked some of the worlds top cities in relation to key factors, like safety, property affordability and climate. Simply select the factors most important to you and we will show you the best cities for you to consider!

For more details visit here [Click here](https://www.movehub.com/city-rankings/)

## We have total 3 files :
...
```{r}
# Frequency of country in dataset 
country <- sort(table(combined_data$Country), decreasing = TRUE)
country_df <- as.data.frame(country)
names(country_df) <- "Size" 
print(country_df, digits = 2, right = TRUE, quote = FALSE)
```

```{r}
# Graphical representation of top 20 countries 
country <- sort(table(combined_data$Country), decreasing = TRUE) 
country_df <- as.data.frame(country)
names(country_df) <- c("Country", "Size")
country_df <- country_df[order(country_df$Size, decreasing = TRUE), ]
top_20_countries <- head(country_df, 20)

# Create the plot 
ggplot(top_20_countries, aes(x = reorder(Country, Size), y = Size)) + 
  geom_bar(stat = "identity", fill = "skyblue") + 
  geom_text(aes(label = Size), vjust = -0.5, color = "black", size = 3) + 
  labs(title = "Top 20 Countries with Highest Number of Observations", x = "Country", y = "Number of Observations") + 
  theme_minimal() + 
...
```{r}
# Are there any significant differences in Cappuccino prices across different countries?
...
```{r}
# Aggregate Cinema prices by country
country_cinema <- combined_data %>%
  group_by(Country) %>%
  summarize(mean_cinema = mean(Cinema))

# Sort the data by mean Cinema prices and select the top 20 countries
top_20_countries <- country_cinema %>%
  arrange(desc(mean_cinema)) %>%
  head(20)

# Create a boxplot to visualize the distribution of Cinema prices across the top 20 countries
ggplot(combined_data %>% filter(Country %in% top_20_countries$Country), aes(x = Country, y = Cinema)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  labs(title = "Distribution of Cinema Prices Across Top 20 Countries",
       x = "Country",
       y = "Cinema Price (USD)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for better readability
...
```{r}
# Plotting the graph of residuals
plotModel(linModel)
...
### Note: The model found with the stepwise regression did not achieve a better R^2 score (0.6886977 vs. 0.6621451)
